...
from shared import tts as tts      
from shared.mode import tts_is_muted     


_TTS_OK = True  # NEW: assume TTS is always OK
...
def _speak_top3(summary_text: str):
    """
    Improved TTS:
    - Groups multiple people: "قدامك شخصين على مسافة قريبة"
    ...
    # 3. Choose items (prefer near/medium, max 3)
    chosen = [it for it in parsed if it[1] in ("near", "medium")] or [parsed[0]]
    chosen = chosen[:3]

    # 4. Build Arabic phrases (people grouped)
    people  = [c for c in chosen if c[0] == "person"]
    others  = [c for c in chosen if c[0] != "person"]
    phrases = []

    if len(people) >= 2:
        same_lvl  = all(p[1] == people[0][1] for p in people)
        same_side = all(p[2] == people[0][2] for p in people)
        if same_lvl and same_side:
            count_ar = {2: "شخصين", 3: "٣ أشخاص"}.get(len(people), f"{len(people)} أشخاص")
            phrases.append(f"{_AR_SIDE[people[0][2]]} {count_ar} على مسافة {_AR_LEVEL[people[0][1]]}")
        else:
            for lab, lvl, side in people:
                phrases.append(f"{_AR_SIDE[side]} {_AR_LABEL[lab]} على مسافة {_AR_LEVEL[lvl]}")
    else:
        for lab, lvl, side in people:
            phrases.append(f"{_AR_SIDE[side]} {_AR_LABEL[lab]} على مسافة {_AR_LEVEL[lvl]}")

    # Add up to two non-person obstacles
    for lab, lvl, side in others[:2 - len(phrases)]:
        phrases.append(f"{_AR_SIDE[side]} {_AR_LABEL[lab]} على مسافة {_AR_LEVEL[lvl]}")

    text = "، و".join(phrases)

    # 5. Speak via streaming TTS
    try:
        tts.enqueue(text)
    except Exception:
        pass
    print("[detector] muted?", tts_is_muted())
